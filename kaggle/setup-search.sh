...
# Superseded GNU build (documented deviation; official GNU build needs glibc >= 2.38, host is 2.35).
...
mkdir -p "$QDRANT_DIR/downloads" "$QDRANT_DIR/bin" "$QDRANT_DIR/storage" \
  "$QDRANT_DIR/snapshots" "$QDRANT_DIR/logs" "$QDRANT_DIR/run" "$EVIDENCE_DIR"

echo "WEMM_SEARCH_SETUP_START qdrant=$QDRANT_VERSION"

[[ -x "$VENV_DIR/bin/python" ]] || { echo "WEMM_SEARCH_SETUP_FAIL: base venv missing" >&2; exit 1; }

# Install search integration deps (qdrant-client, fastapi, uvicorn, httpx, pyarrow)
# without ever touching the frozen Torch stack.
if grep -Eiv '^[[:space:]]*(#|$)' "$ROOT/requirements-search.txt" \
  | grep -Ei '(^|[[:space:]])(torch|triton|nvidia[-_]|cuda[-_])' >/dev/null; then
  echo "WEMM_SEARCH_SETUP_FAIL: forbidden Torch/CUDA/NVIDIA dependency in requirements-search.txt" >&2
  exit 1
fi
"$PYTHON_BIN" -m pip --python "$VENV_DIR/bin/python" install \
  --disable-pip-version-check --no-cache-dir -r "$ROOT/requirements-search.txt"

# Download / verify / extract pinned Qdrant 1.19.0 musl static build.
ARCHIVE_PATH="$QDRANT_DIR/downloads/$MUSL_ARCHIVE"
if [[ ! -f "$ARCHIVE_PATH" ]]; then
  echo "WEMM_SEARCH_DOWNLOAD url=$MUSL_URL"
  curl -fL --retry 3 -o "$ARCHIVE_PATH" "$MUSL_URL"
fi
echo "$MUSL_SHA256  $ARCHIVE_PATH" > "$QDRANT_DIR/downloads/$MUSL_ARCHIVE.sha256"
(cd "$QDRANT_DIR/downloads" && sha256sum -c "$MUSL_ARCHIVE.sha256")
...
echo "WEMM_SEARCH_SETUP_PASS bin=$BIN_PATH"
