...
def test_eos_pooling_rejects_left_padding_tokenizer():
    from wemm_kaggle.embedding import ensure_right_padding

    processor = SimpleNamespace(
        tokenizer=SimpleNamespace(padding_side="left")
    )
    with pytest.raises(RuntimeError, match="padding_side"):
        ensure_right_padding(processor)


def test_eos_pooling_accepts_right_padding_tokenizer():
    from wemm_kaggle.embedding import ensure_right_padding

    processor = SimpleNamespace(
        tokenizer=SimpleNamespace(padding_side="right")
    )
    ensure_right_padding(processor)  # must not raise


def test_eos_pooling_is_agnostic_when_tokenizer_is_absent():
    from wemm_kaggle.embedding import ensure_right_padding

    ensure_right_padding(SimpleNamespace())  # must not raise


def test_batched_encoding_enforces_right_padding_contract():
    runtime, _, _, _ = make_runtime()
    runtime.processor.tokenizer = SimpleNamespace(padding_side="left")
    with pytest.raises(RuntimeError, match="padding_side"):
        runtime.embed_texts(["hello", "xin chao"])

def test_padding_evidence_left_side_is_fail(capsys):
    from wemm_kaggle.embedding import emit_padding_evidence

    line = emit_padding_evidence(
        SimpleNamespace(tokenizer=SimpleNamespace(padding_side="left"))
    )
    captured = capsys.readouterr()
    assert "WEMM_TOKENIZER_PADDING_SIDE=left" in line
    assert "WEMM_TOKENIZER_PADDING_CONTRACT=FAIL" in line
