...
def test_shutdown_during_model_load_never_returns_service_to_ready():
    ...
    def _slow_loader():
        loader_started.set()
        assert release_loader.wait(timeout=10), "loader was not released in time"
        return fake.load()

    sched = InferenceScheduler(_slow_loader)

    async def wait_phase(target):
        for _ in range(1000):
            if sched.state.phase == target:
                return
            await asyncio.sleep(0.01)
        raise AssertionError(f"phase never became {target.value}; got {sched.state.phase}")

    async def main():
        start_task = asyncio.create_task(sched.start())
        assert await asyncio.to_thread(loader_started.wait, 10), "runtime loader did not start"
        shutdown_task = asyncio.create_task(sched.shutdown())
        await wait_phase(ServicePhase.DRAINING)
        release_loader.set()
        await shutdown_task
        await start_task
        assert sched.state.phase == ServicePhase.STOPPED
        assert sched._worker_task is None
        with pytest.raises(ServiceNotReadyError):
            await sched.submit("text", ["a"], 4096)
        await sched.shutdown()
        await sched.shutdown()
