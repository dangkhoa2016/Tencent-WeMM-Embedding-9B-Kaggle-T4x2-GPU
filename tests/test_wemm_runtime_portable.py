...
def test_wemm_runtime_embedding_mirror_enforces_right_padding_contract():
    from wemm_runtime.embedding import ensure_right_padding

    ensure_right_padding(SimpleNamespace())
    ensure_right_padding(SimpleNamespace(tokenizer=SimpleNamespace(padding_side="right")))
    with pytest.raises(RuntimeError, match="padding_side"):
        ensure_right_padding(
            SimpleNamespace(tokenizer=SimpleNamespace(padding_side="left"))
        )


def test_padding_evidence_right_side_is_pass(capsys):
    from wemm_runtime.embedding import emit_padding_evidence

    line = emit_padding_evidence(
        SimpleNamespace(tokenizer=SimpleNamespace(padding_side="right"))
    )
    captured = capsys.readouterr()
    assert "WEMM_TOKENIZER_PADDING_SIDE=right" in line
    assert "WEMM_TOKENIZER_PADDING_CONTRACT=PASS" in line
    assert "WEMM_TOKENIZER_PADDING_SIDE=right" in captured.out
    assert "WEMM_TOKENIZER_PADDING_CONTRACT=PASS" in captured.out


def test_padding_evidence_unknown_side_is_not_proven(capsys):
    from wemm_runtime.embedding import emit_padding_evidence

    line = emit_padding_evidence(SimpleNamespace())
    captured = capsys.readouterr()
    assert "WEMM_TOKENIZER_PADDING_SIDE=UNKNOWN" in line
    assert "WEMM_TOKENIZER_PADDING_CONTRACT=NOT_PROVEN" in line
    assert "UNKNOWN" in captured.out
    assert "NOT_PROVEN" in captured.out
