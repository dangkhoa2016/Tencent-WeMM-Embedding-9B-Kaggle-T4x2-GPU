...
def test_frozen_runtime_and_visual_contracts_are_not_reopened():
    ...
    assert "224f07cd1d6eb174d3532c9eaeeb9abd606a857f" in bootstrap
    assert 'PRESENTATION_ROOT = Path(__file__).resolve().parents[1]' in bootstrap
    assert 'str(RUNTIME_ROOT / "requirements-kaggle.txt")' in bootstrap
    assert 'str(PRESENTATION_ROOT / "requirements-demo.txt")' in bootstrap
    assert 'str(RUNTIME_ROOT / "requirements-demo.txt")' not in bootstrap
...
def test_visual_transform_preview_is_auditable_and_presentation_only():
    source = (MODULE_DIR / "visual_showcase.py").read_text(encoding="utf-8")

    assert "def _display_transform_evidence(" in source
    ...
    assert "CROP BOUNDARY (VISUAL AID)" in source
    assert "VISUALIZATION ONLY: contact sheet + crop boundary are not used for embedding/retrieval." in source
    ...
    assert "embed_image(sheet" not in source
    assert "embed_image(crop_overlay" not in source
...
def test_public_notebook_markdown_restores_kaggle_onboarding_and_phase_guidance():
    ...
    assert "**WeMM-Embedding-9B**" in intro
    assert "WeChat Vision Team at Tencent" in intro
    assert "đội ngũ WeChat Vision tại Tencent" in intro
    assert "Tencent WeMM-Embedding-9B" not in intro
    assert "Add Input" not in intro
...
def test_public_notebook_omits_internal_corrective_recovery_metadata():
    nb = _notebook()
    text = NOTEBOOK.read_text(encoding="utf-8")
    for forbidden in (
        "science_" + "reopened",
        "source_saved_" + "version",
        "succ" + "essor",
        "corrective_" + "scope",
    ):
        assert forbidden not in text
    metadata = nb["metadata"]["wemm_public_demo"]
    assert metadata["runtime_commit"]
    assert metadata["presentation_source_ref"] == "v1.0.0"
