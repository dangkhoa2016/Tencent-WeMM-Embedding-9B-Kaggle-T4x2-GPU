...
Check that the model input is attached and contains the required WeMM-Embedding-9B model files. Use `KAGGLE_MODEL_DIR` only when automatic discovery is ambiguous.
...
## Public notebook bootstrap cannot find `requirements-demo.txt`

If setup fails with a path similar to:

```text
/kaggle/working/wemm-production-runtime-<sha>/requirements-demo.txt
```

do not copy the file into the frozen runtime checkout. The public notebook keeps dependency authority split intentionally: `requirements-kaggle.txt` belongs to the frozen runtime, while `requirements-demo.txt` belongs to the current public presentation source. A failure that looks for the demo requirements under the frozen runtime is a presentation/bootstrap source-path defect.
...
The in-process model runtime may still be initializing behind the scheduler. Check `/readyz` and server logs.
