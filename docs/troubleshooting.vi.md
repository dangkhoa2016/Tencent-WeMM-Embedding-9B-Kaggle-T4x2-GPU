...
Kiểm tra model input đã attach và có đủ các file model WeMM-Embedding-9B bắt buộc. Chỉ dùng `KAGGLE_MODEL_DIR` khi automatic discovery bị mơ hồ.
...
## Public notebook bootstrap không tìm thấy `requirements-demo.txt`

Nếu setup fail với path dạng:

```text
/kaggle/working/wemm-production-runtime-<sha>/requirements-demo.txt
```

không copy file này vào frozen runtime checkout. Public notebook chủ ý tách dependency authority: `requirements-kaggle.txt` thuộc frozen runtime, còn `requirements-demo.txt` thuộc current public presentation source. Nếu bootstrap tìm demo requirements dưới frozen runtime thì đó là lỗi source path của presentation/bootstrap.
...
In-process model runtime có thể vẫn đang initialize phía sau scheduler. Kiểm tra `/readyz` và server logs.
