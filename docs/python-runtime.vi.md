...
Model configuration của WeMM-Embedding-9B phải expose upstream custom model code bắt buộc và advertise Matryoshka dimension 4096.
...
Khi phát hiện multi-GPU device map, runtime cài một embedding-dispatch compatibility path nhỏ quanh model WeMM-Embedding-9B để embedding call đi theo placement đã validate.

## Resource lifecycle

Public notebook/demo integration bọc runtime trong `SubprocessEmbeddingWorker` để trạng thái GPU có thể được thu hồi có kiểm soát khi demo closeout. Nếu dùng trực tiếp `wemm_runtime/`, người tích hợp phải tự quản lý lifecycle model/process.

FastAPI loopback hiện giữ model runtime trong cùng process của service và đặt nó phía sau `InferenceScheduler` một luồng; API không đưa inference qua subprocess worker của notebook/demo.
