...
Trong public notebook bootstrap, dependency ownership được tách rõ: `requirements-kaggle.txt` được đọc từ frozen runtime checkout, còn `requirements-demo.txt` được đọc từ public presentation source checkout tại `v1.0.0`. Không copy `requirements-demo.txt` vào frozen runtime tree để workaround.
...
## Bằng chứng tokenizer padding

Qualified runtime nạp processor thật từ `/kaggle/input`. Khi load model, runtime phát ra marker bằng chứng deterministic:
...
Right-padding contract là bắt buộc cho EOS pooling. Nếu tokenizer được nạp không expose `padding_side`, runtime phát ra:
...
Marker not-proven không được coi là qualified. Giá trị `left` (hoặc batch behavior mâu thuẫn với right padding) đòi hỏi một qualification phase T4×2 mới; thay đổi embedding-science không được âm thầm patch.
