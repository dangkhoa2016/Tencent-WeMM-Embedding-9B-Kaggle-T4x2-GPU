...
REPO_BLOB_MAIN="https://github.com/dangkhoa2016/Tencent-WeMM-Embedding-9B-Kaggle-T4x2-GPU/blob/main"
COMMUNITY_HEALTH={"CONTRIBUTING.md","CODE_OF_CONDUCT.md","SECURITY.md","SUPPORT.md"}
...
def banner(path):
    ...
    if n.endswith(".vi.md"):
        other=n[:-6]+".md"
        target=other
        if p.parent==Path(".github") and other in COMMUNITY_HEALTH:
            target=f"{REPO_BLOB_MAIN}/.github/{other}"
        return f"> 🌐 Language / Ngôn ngữ: [English]({target}) | **Tiếng Việt**"
    other=n[:-3]+".vi.md"
    target=other
    if p.parent==Path(".github") and n in COMMUNITY_HEALTH:
        target=f"{REPO_BLOB_MAIN}/.github/{other}"
    return f"> 🌐 Language / Ngôn ngữ: **English** | [Tiếng Việt]({target})"
def main():
