...
def _display_transform_evidence(original_path: Path, transform_paths: dict[str, Path]):
    """Render a human-auditable contact sheet without changing retrieval inputs."""
    ...
    for transform_name in (
        ...
        transform_path = transform_paths[transform_name]
        with Image.open(transform_path) as transformed:
            transformed_rgb = transformed.convert("RGB")
        panels.append((transform_name, transformed_rgb, transformed_rgb.size))

        transform_sha = _sha256(transform_path)
        print(
            f"    {transform_name:<20} | size={list(map(int, transformed_rgb.size))} | "
            f"sha256={transform_sha} | changed-from-original="
            f"{'YES' if transform_sha != original_sha else 'NO'}",
            flush=True,
        )

    crop_overlay = original.copy()
    crop_draw = ImageDraw.Draw(crop_overlay)
    width, height = crop_overlay.size
    dx = max(1, int(width * 0.02))
    dy = max(1, int(height * 0.02))
    line_width = max(2, int(min(width, height) * 0.006))
    crop_draw.rectangle(
        (dx, dy, width - dx - 1, height - dy - 1),
        outline=(255, 0, 0),
        width=line_width,
    )
    ...
    for panel_index, (title, image, actual_size) in enumerate(panels):
        ...
        target_h = max(1, int(image.size[1] * base_scale))
        rendered = image.resize((target_w, target_h), Image.Resampling.LANCZOS)
    ...
    print(
        "  VISUALIZATION ONLY: contact sheet + crop boundary are not used for embedding/retrieval.",
        flush=True,
    )
    display(sheet)
...
def run_visual_showcase(demo) -> dict:
    ...
    try:
        ...
        for index, rec in enumerate(assets, 1):
            ...
            print(
                "  Bằng chứng transform / Transform preview evidence "
                "(all 4 frozen transforms):",
                flush=True,
            )
            _display_transform_evidence(Path(rec["image_path"]), transform_paths)
        ...
        print("VISUAL_TRANSFORM_PREVIEW_EVIDENCE=PASS")
