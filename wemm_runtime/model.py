...
from .embedding import build_messages, pool_eos_embedding, truncate_mrl
...
def validate_model_dir(path: Path) -> ModelIdentity:
    ...
    if missing:
        raise ValueError(
            f"Incomplete WeMM model directory {path}; missing: {', '.join(missing)}"
        )
    ...
    if 4096 not in dims:
        raise ValueError(
            f"WeMM-Embedding-9B config must advertise Matryoshka dimension 4096: {path}"
        )
