...
def validate_model_dir(path: Path) -> ModelIdentity:
    ...
    if missing:
        raise ValueError(f"Incomplete Tencent WeMM-Embedding-9B model directory {path}; missing: {', '.join(missing)}")
    ...
    if 4096 not in dims:
        raise ValueError(f"Tencent WeMM-Embedding-9B config must advertise Matryoshka dimension 4096: {path}")
...
def resolve_model_dir(root: Path = Path("/kaggle/input"), explicit: str | None = None) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(
            "No complete Tencent WeMM-Embedding-9B model found under /kaggle/input. Attach the Kaggle Model first."
        )
    if len(candidates) > 1:
        rendered = "\n  - ".join(str(p) for p in candidates)
        raise RuntimeError(
            "Multiple complete Tencent WeMM-Embedding-9B model candidates found; set KAGGLE_MODEL_DIR to one exact mounted path:\n  - "
            + rendered
        )
    return candidates[0]
